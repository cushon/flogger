...
final class StackGetterTestUtil {
  ...
  static StackGetter maybeCreateStackGetter(String className) {
    try {
      return Class.forName(className)
        ...
        | LinkageError expected) {
      // this is expected to happen in cases when the test runs in an env where one or the other of
      // the StackGetter implementations are not available.
    }
    return null;
  }
}
